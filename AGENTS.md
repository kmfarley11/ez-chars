...
- **Remote Model Disclosure:** Before directly inspecting nonpublic, paid, rights-restricted, or personally sensitive local source content when the agent cannot verify the client/provider's transmission, retention, or training controls, briefly warn the human that selected content or tool output may be sent to a remote model service and ask them to confirm their settings and authorization. Do not claim that provider processing necessarily makes the content public or trains a public model. Once the human accepts that risk for the scoped task, proceed without repeating the warning; sanitized bibliographic metadata does not require a new warning.
...
- Keep redistributed document paths, official downloads, checksums, review dates, modification statuses, licenses, and exact attribution centralized in [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md); other docs should link to that user-facing inventory instead of duplicating its metadata.
...
- **Generated OpenSpec Instructions**: Treat `.codex/skills/openspec-*` and equivalent tool-specific OpenSpec workflow files as generated output owned by the pinned `@fission-ai/openspec` dependency. Do not hand-edit them for repository policy. Keep project-specific workflow overrides in this `AGENTS.md`; after intentionally updating the dependency or OpenSpec profile, run `openspec update` and review the regenerated files. The overrides below should be a priority to extend the installed skills rather than to update the skills themselves. If a user dictates that instruction wasn't followed, a custom skill-wrapper should be considered. We should never edit the installed skills themselves that come directly from openspec.
- **Workflow Skill Overrides**: When executing OpenSpec workflows (explore, propose, apply, archive), agents MUST follow these repository-specific rules over the generic skill instructions:
  - **Explore**: If a feature idea is refined but no proposal is started, explicitly offer to capture the "Refined feature idea" into the `docs/backlog.md` priority queue. Also look for a proof-before-propagation opportunity: when a small isolated Storybook story, prototype, representative fixture, migration dry run, or sample output could let the human owner validate a consequential shape before broad rollout, recommend a named mid-apply checkpoint and identify what later work should wait. Do not add a checkpoint when review cannot materially change the downstream implementation.
  - **Propose**: When a proof-before-propagation checkpoint is justified, describe its proof and rollout boundary in `design.md`, then structure `tasks.md` as a pre-gate proof batch, a named unchecked `STOP — Human review and approval` task, and the dependent rollout batch. The stop task must say what the owner reviews and what explicit approval unlocks; workflow gates do not belong in behavioral specs. Always ensure the final task section separately includes (1) a named post-apply user review and explicit approval task that agents cannot self-complete and (2) explicit archive-time tasks to prune the completed backlog item from `docs/backlog.md` and re-sequence the "Next recommended sequence" priority queue block.
  - **Apply**: When the next pending task is a named mid-apply human checkpoint, present the isolated proof, inspection instructions, decisions under review, and completed verification, then stop without checking the task or starting dependent work. Only explicit approval of that checkpoint permits the agent to check it and resume; requested revisions remain in the pre-gate batch. Separately, agents may complete implementation, reconciliation, and verification tasks, but MUST leave the named final post-apply user review/approval task unchecked. After presenting the resulting change scope, verification evidence, and material fallout, stop for explicit human approval. A request to begin applying the change, a clean self-review, or passing automation is not approval of either checkpoint.
  - **Archive**:
...
The intent of our coding flows is to lean into agentic execution while heavily relying on human review of docs and particular gates. Since agents generate content quickly, you must provide concise, high-value summaries to accelerate the human's review and health checks.

When the user asks you to review changes (or proposal artifacts), specifically:
...
- **"Review the changes"**: Perform a standard code review. Look for correctness, logical errors, standard best practices, and include the Top 5 highlights.
- **"Strategically review the changes"** (or any phrasing indicating another agent wrote the code, or that a thorough analysis is necessary): Act as a strategic reviewer and synthesizer. Do all typical code review exercises but with an eye for scrutiny for cohesion in particular. i.e.:
  - **Assess Alignment:** Evaluate whether the changes remain coherent with the project's vision and the owner's explicit intent (as defined in `docs/vision/author-desires.md`, `docs/backlog.md`, or the specific OpenSpec artifacts).
