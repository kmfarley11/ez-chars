...
**Reason**: No character layout predating the new v0 baseline has a user-facing compatibility promise. Retaining those experimental transformations increases implementation and agent context during rapid pre-playtest schema discovery.

**Migration**: Earlier experimental local or imported data is rejected through the existing invalid/outdated-data path without being overwritten. Git history, archived OpenSpec changes, and ADRs preserve the schema evolution rationale. The first real playtest will establish an immutable v1 baseline and begin durable sequential migration support.
