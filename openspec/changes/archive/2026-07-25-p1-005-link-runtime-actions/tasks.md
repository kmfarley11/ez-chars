...
- [x] 4.7 Add a feature-local row projection and render each runtime action once with name, timing, category, optional target metadata, and non-empty notes as secondary italic content.
- [x] 4.8 Retain the existing collection-level `GridContent` Edit and Notes workflows without duplicating the runtime summary; add only a semantic-free controls-only presentation option if composition requires it, with no per-row editing or annotation contract.
- [x] 4.9 Remove the separate action-source status list and place View-source and Resync commands in one compact, keyboard-accessible menu on each linked action row; custom actions receive no source menu or persistent status label.
- [x] 4.10 Update the stateful Storybook playground and deterministic scenario stories to cover visible authored notes, retained card-level Edit/Notes workflows, compact linked-source commands, custom actions, and suggestion request states.
...
- [x] 5.4 Add focused projection/component contracts and update the Chromium flow to prove authored notes remain visible, source commands navigate/resync, card-level Edit/Notes remain available, and deletion fallback removes source controls without removing the action.
- [x] 5.5 Re-run `npm run verify:smoke`, `npm run test:coverage`, `npm run build`, `npm run build-storybook`, and strict OpenSpec validation after the row-oriented refinement.
- [x] 5.6 Update default UI seed data and Playwright fixtures so representative current-schema characters include one source-linked runtime action and one fully custom action; add fixture/browser assertions that keep the distinction visible.
...
- [x] 6.5 Reconcile the runtime-action ADR, active-goal wording, and provisional `p1-005` Done Recently entry with the exact row-oriented implementation while preserving both `p1-061` and the generic collection-interaction backlog follow-up.
