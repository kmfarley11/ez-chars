...
- equipped inventory items can seed independently editable runtime-action snapshots through a narrow 5e suggestion boundary; linked actions expose source navigation and explicit name/notes resync, then fall back to preserved custom actions if their source item is deleted
