...
- The 5e runtime-action component projects one player-facing row per action, with timing/category/target metadata and non-empty authored notes kept visible. Linked rows group source navigation and explicit resync in a compact source menu; custom rows carry no persistent status label.
- The existing collection-level Edit and Notes workflows remain authoritative. BL-074 later removed the temporary controls-only `GridContent` presentation while preserving the same boundary: the 5e runtime-action organism owns its row, source, and navigation semantics and composes generic action/dialog pieces without moving those semantics into the shared grid API.
- The route continues to own inventory-group navigation and focus. The broader collection-row editing and annotation interaction model remains a separate backlog exploration rather than being standardized through this inventory slice.
...
- No material deviation widens the approved boundary. `p1-062` (guided templating dialog) becomes the next recommended P1 item to validate the UX before `p1-061` resolves spell/feature identity and eligibility to widen the source union.

**2026-07-25: Completing On-Sheet Multi-Source Actions**

- **Pre-Playtest Baseline:** The canonical character layout will rebase to the explicitly unstable `dnd5e-2014.schema.v0` epoch defined by the schema-versioning ADR. Earlier experimental layouts are rejected rather than migrated; current seeds, fixtures, imports, and newly created records must satisfy the complete v0 shape directly.
- **Stable Character-Owned Identity:** Every spell and persisted feature reference requires a stable ID. Spell IDs are unique within the spell collection, and feature IDs are unique across general, ancestry, background, class, and subclass feature storage so the atomic `{ kind: 'feature', id }` reference resolves without owner-specific fields. Current validation rejects missing or colliding IDs instead of inventing or repairing them during hydration.
- **Eligible Sources:** Runtime actions may be created from inventory items, any spell on the character sheet, general features, class features, subclass features, and ancestry traits. Background features remain ineligible until their user-facing presentation is refined. Traits continue to appear separately from Features even though both use the persisted `feature` source kind.
- **Sheet Presentation Without Premature Normalization:** The sheet presents general, class, and subclass entries as one Features collection, while the persisted records remain in their existing owners. A non-persisted ownership discriminator routes edits. New manually authored Features use the top-level general collection. Normalizing feature provenance into a new storage model remains deferred.
- **Synchronous Local Candidate Composition:** The guided picker consumes one normalized, 5e-specific candidate projection built from character-owned records. Because every eligible record is already local, this path is synchronous and the speculative loading and request-error states introduced by the inventory seam are removed. This supersedes the earlier expectation that the on-sheet workflow should preserve an asynchronous calling convention.
- **External Compendium Boundary:** Future compendium results must first become character-owned records before they can source runtime actions. Network lookup, disambiguation, and offline states therefore belong to the future compendium-acquisition workflow rather than the local action picker.
- **Text Ownership:** Inventory items and spells own the action name and notes. General features own the action name and the first non-empty feature summary or description as notes. Class features, subclass features, and ancestry traits own only the action name, preserving action-authored notes during resynchronization.
- **Explicit Resynchronization:** Actions remain editable snapshots. Resynchronization requires confirmation that source-owned content may be overwritten, then replaces only the fields owned by that source kind. Override masks, append-versus-overwrite modes, and retrospective linking of custom actions remain deferred until their concrete workflows are refined.
- **Guided and Custom Entry:** One Add action command opens the paged dialog. The first page supports searchable, categorized selection from all eligible sources or immediate custom entry; the second page reviews one focused action draft. Search and categories are reusable picker behavior, while the complete runtime-action workflow remains an organism-level composition.

**2026-07-26: Multi-Source Implementation Outcome**

- The strict `dnd5e-2014.schema.v0` source union now accepts item, spell, and feature references. Traits use the feature kind, and the current schema rejects missing, dangling, ineligible, or ambiguous linked identities.
- One synchronous 5e domain module owns normalized candidate projection, filtering, resolution, draft seeding, source-owned text, and source destinations. The former inventory-only asynchronous suggestion seam was removed; no generic registry or speculative network lifecycle was introduced.
- The sheet combines general/manual and class/subclass entries in Features by using non-persisted ownership metadata for edit routing. Traits remain a separate structured collection, and background features remain ineligible as action sources.
- The Add action dialog now searches and filters all eligible local sources or starts a custom draft, then reviews one action before commit. Source switching reseeds the draft, while ordinary back/forward navigation preserves it.
- Linked and custom rows use the shared source-category projection to pronounce Inventory, Spell, Feature, Trait, or Custom directly in runtime metadata. Linked rows use the shared resolver for source-card navigation; resync requires confirmation and replaces only source-owned text, while name-only class/subclass features and Traits preserve action-authored notes. Removing any eligible source unlinks affected actions atomically without changing their snapshots.
- Inventory grouping moved into a reusable 5e domain module so source projection does not depend on route-owned presentation code. This is a feature-local extraction, not a new cross-system adapter.
