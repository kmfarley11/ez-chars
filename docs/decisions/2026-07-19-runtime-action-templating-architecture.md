...
- The existing collection-level Edit and Notes workflows remain authoritative. `GridContent` gained only a semantic-free controls-only presentation so the 5e component can retain those workflows without duplicating its row summary; no per-row editing or source semantics entered the generic grid API.
